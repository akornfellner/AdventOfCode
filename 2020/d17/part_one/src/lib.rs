...
fn cubes_from_input(input: &str) -> HashMap<(i32, i32, i32), bool> {
    let rows: Vec<&str> = input.split('\n').collect();
    let mut result: HashMap<(i32, i32, i32), bool> = HashMap::new();
    for i in 0..rows.len() {
        let chars: Vec<char> = rows[i].chars().collect();
        for j in 0..chars.len() {
            if chars[j] == '#' {
                result.insert((i as i32, j as i32, 0), true);
            }
        }
    }
...
fn get_neighbors(cube: &(i32, i32, i32)) -> Vec<(i32, i32, i32)> {
    ...
    for x in -1..2 {
        for y in -1..2 {
            for z in -1..2 {
                if !(x == 0 && y == 0 && z == 0) {
                    result.push((cube.0 + x, cube.1 + y, cube.2 + z));
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn it_works() {
        assert_eq!(d17("input_test.txt"), 112);
    }
}
