...
pub fn solve(filename: &str) -> i32 {
    ...
    let result = visit("start", &caves, &visited);

    result
}

fn visit(cave: &str, caves: &HashMap<&str, Vec<&str>>, visited: &Vec<&str>) -> i32 {
...
mod tests {
    ...
    #[test]
    fn it_works() {
        assert_eq!(solve("input_test.txt"), 10);
    }
}
